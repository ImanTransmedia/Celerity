...
namespace TS.PageSlider
{
    public class PageSlider : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            GameObject[] objectsWithTag = GameObject.FindGameObjectsWithTag("Slides");

            foreach (var item in objectsWithTag)
            {
                PageContainer contenedores = item.GetComponent<PageContainer>();

                _pages.Add(item.GetComponent<PageContainer>()) ;
            }
